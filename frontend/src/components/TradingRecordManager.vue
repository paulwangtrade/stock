...
import { quantChecklistFor } from '../utils/quantAutomationStore'
...
function query({ page, pageSize = 12, keyword = '', direction = '', status = '', startDate = '', endDate = '' }) {
...
function silentRefreshCurrentPage() {
  query(buildListQuery(paginationReactive.page))
    .then((data) => {
...
function handlePageChange(currentPage) {
  if (!loadingRef.value) {
    ...
    query(buildListQuery(currentPage))
      .then((data) => {
        dataRef.value = data.data
...
function confirmDraft(row) {
  if (row.Direction === '买入') {
    const gate = evaluateTradeRiskGate({
      ...
      checklist: quantChecklistFor(row.StockCode),
      direction: '买入',
    })
    if (!gate.ok) {
      message.error(gate.reason || '风控拦截')
      return
    }
  }
  ConfirmTradingRecordDraft(row.ID)
    .then(() => {
...
const columnsRef = ref([
  ...
  },
  {
    ...
    render(row) {
      ...
      if (st === 'draft') {
        return h(NTag, { type: 'warning', size: 'small', round: true, bordered: false }, { default: () => '草稿' })
      }
      return h(NText, { depth: 3 }, { default: () => '已确认' })
    }
  },
  {
    title: '价格',
  ...
  },
  {
    ...
    render(row) {
      const actions = [
        h(NTag, { strong: true, tertiary: true, type: 'info', onClick: () => openKlineChart(row) }, { default: () => 'K线' }),
      ]
      if ((row.Status || 'confirmed') === 'draft') {
        actions.push(
          h(NTag, { strong: true, tertiary: true, type: 'success', onClick: () => confirmDraft(row) }, { default: () => '确认' }),
        )
      }
      actions.push(
        h(NTag, { strong: true, tertiary: true, type: 'warning', onClick: () => openEditModal(row) }, { default: () => '编辑' }),
        h(NTag, { strong: true, tertiary: true, type: 'error', onClick: () => deleteTradingRecord(row.ID) }, { default: () => '删除' }),
      )
      return actions
    }
  }
...
onMounted(() => {
  ...
  EventsOn('tradingRecordDraftUpdated', () => silentRefreshCurrentPage())
})

onUnmounted(() => {
  EventsOff('tradingRecordDraftUpdated')
  // 清除定时器
...
<template>
  ...
  <stock-kline-modal
    ...
    :title="'K线 - ' + klineStockName"
    modal-width="min(1400px, 95vw)"
    :chart-key="'trading-record-kline-' + klineStockCode"
    :code="klineStockCode"
    :stock-name="klineStockName"
    :dark-theme="darkTheme"
    :long-stop-loss-price="longStopLossPrice"
    :long-take-profit-price="longTakeProfitPrice"
