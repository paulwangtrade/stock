...
function dedupeTelegraphList(list) {
  ...
  for (const item of list) {
    ...
    const key = `${item.source || ''}|${item.time || ''}|${content.slice(0, 100)}`
...
function applyRouteQuery() {
  const tab = route.query.name
  if (tab) nowTab.value = String(tab)
  if (route.query.indexTab) majorIndexTab.value = String(route.query.indexTab)
  if (route.query.stockCode) stockCode.value = String(route.query.stockCode)
}
...
onBeforeMount(() => {
  ...
  GetTelegraphList("财联社电报").then((res) => {
    telegraphList.value = dedupeTelegraphList(res)
  })
  GetTelegraphList("新浪财经").then((res) => {
    sinaNewsList.value = dedupeTelegraphList(res)
  })
  GetTelegraphList("外媒").then((res) => {
    foreignNewsList.value = dedupeTelegraphList(res)
  })
  getIndex();
  industryRank();
  startMarketPolling()
...
function shouldPollMarket() {
  return !document.hidden && route.name === 'market'
}
...
function onMarketVisibilityChange() {
  if (shouldPollMarket()) {
    getIndex()
  }
}
...
onMounted(() => {
  document.addEventListener('visibilitychange', onMarketVisibilityChange)
})


onBeforeUnmount(() => {
  document.removeEventListener('visibilitychange', onMarketVisibilityChange)
...
watch(
  () => route.name,
  (name) => {
    if (name === 'market' && !document.hidden) {
      getIndex()
    }
  },
)
...
EventsOn("changeMarketTab", async (msg) => {
  ...
  updateTab(msg.name)
  if (msg.indexTab) majorIndexTab.value = String(msg.indexTab)
})

watch(
  () => [route.query.name, route.query.indexTab, route.query.stockCode],
  () => applyRouteQuery(),
)

EventsOn("newTelegraph", (data) => {
  if (data != null && data.length) {
    telegraphList.value = dedupeTelegraphList([...data, ...telegraphList.value]).slice(0, 50)
  }
})
EventsOn("newSinaNews", (data) => {
  if (data != null && data.length) {
    sinaNewsList.value = dedupeTelegraphList([...data, ...sinaNewsList.value]).slice(0, 50)
  }
})
EventsOn("tradingViewNews", (data) => {
  if (data != null && data.length) {
    foreignNewsList.value = dedupeTelegraphList([...data, ...foreignNewsList.value]).slice(0, 50)
  }
})
  ...
  <n-card>
    <n-tabs type="line" animated @update-value="updateTab" :value="nowTab" style="--wails-draggable:no-drag">
      ...
      <n-tab-pane name="重大指数" tab="重大指数">
        <n-tabs type="segment" animated :value="majorIndexTab" @update:value="(v) => majorIndexTab = v">
