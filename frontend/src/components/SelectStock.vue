...
import {Environment, EventsEmit} from "../../wailsjs/runtime"
...
function saveAsStrategy() {
  if (!search.value?.trim()) {
    message.warning('请先输入选股条件')
    return
  }
  EventsEmit('requestSaveStockStrategy', {
    queryType: 'eastmoney_nl',
    queryText: search.value.trim(),
  })
}
    ...
    <n-gi :span="20">
      <n-flex style="--wails-draggable:no-drag">
        <n-input-group style="text-align: left">
          ...
          <n-button tertiary type="warning" @click="saveAsStrategy">保存为策略</n-button>
        </n-input-group>
      </n-flex>
