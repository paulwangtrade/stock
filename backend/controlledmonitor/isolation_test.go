...
func TestIsolation_NoTradeChainWritesOrProviderSwitch(t *testing.T) {
	wd, err := os.Getwd()
...
func TestIsolation_WriteChainDoesNotImportControlledMonitor(t *testing.T) {
	targets := []string{
	...
	for _, rel := range targets {
		...
		require.NoError(t, err)
		require.NotContains(t, string(src), "controlledmonitor", path)
	}
}
