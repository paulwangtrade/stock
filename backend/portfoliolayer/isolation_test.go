	...
	for _, e := range entries {
		...
		for _, imp := range f.Imports {
			...
			for _, bad := range forbiddenImports {
				require.NotEqual(t, bad, path, "%s imports trading-chain package %s", e.Name(), bad)
			}
		}
	}
}

func TestTradingChain_DoesNotImportPortfolioLayer(t *testing.T) {
	t.Parallel()
	repo := repoRoot(t)
	for _, rel := range tradingChainFiles {
		...
		require.NotContains(t, string(b), "go-stock/backend/portfoliolayer", "%s must not import portfoliolayer", rel)
		require.NotContains(t, string(b), "portfoliolayer.", "%s must not call portfoliolayer", rel)
	}
}
