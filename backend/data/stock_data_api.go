...
type StockInfo struct {
	...
	FollowPrice       float64 `json:"关注价格"`              // 加入自选时的价格（基准价）
	FollowProfit      float64 `json:"followProfit"`      // 关注至今涨跌幅（%）

	Sort               int64   `json:"sort"` //排序
	AlarmChangePercent float64 `json:"alarmChangePercent"`
	AlarmPrice         float64 `json:"alarmPrice"`
	FollowTime         string  `json:"关注时间"` // 加入自选时间
	Industry           string  `json:"所属行业"`
	BKName             string  `json:"所属板块"`
...
type FollowedStock struct {
	...
	FollowPrice        float64 // 加入自选时的价格，不随行情更新
}
...
type TradingRecord struct {
	...
	Status          string `gorm:"index;default:confirmed"` // confirmed / draft
	Price           float64
	Volume          int64
...
type TradingRecordListQuery struct {
	...
	Status    string `json:"status"`    // confirmed / draft，空表示全部
...
func (receiver StockDataApi) Follow(stockCode string) string {
	...
	maxFollow := GetMaxFollowCount()
	if count >= int64(maxFollow) {
		return fmt.Sprintf("最多只能关注%d只股票", maxFollow)
	}
...
// LookupFollowBaselinePrice 按关注日期取日 K 收盘价作为基准价（历史自选回填用）。
func (receiver StockDataApi) LookupFollowBaselinePrice(stockCode string, followTime time.Time) float64 {
	if followTime.IsZero() {
		return 0
	}
	targetDay := normalizeKLineDayKey(followTime.Format("2006-01-02"))
	api := NewEastMoneyKLineApi(receiver.config)
	klines := api.GetKLineDataBefore(stockCode, "101", "", 160, followTime.Format("20060102"))
	if klines == nil || len(*klines) == 0 {
		return 0
	}
	var chosen float64
	for _, k := range *klines {
		day := normalizeKLineDayKey(k.Day)
		closeP, err := convertor.ToFloat(k.Close)
		if day == "" || err != nil || closeP <= 0 {
			continue
		}
		if day == targetDay {
			return closeP
...
// ResolveFollowBaselinePrice 解析自选盈亏基准价：优先关注日收盘价，必要时修正错误回填的关注价。
func (receiver StockDataApi) ResolveFollowBaselinePrice(follow FollowedStock, currentPrice float64) float64 {
	stored := follow.FollowPrice
	...
	// 已有关注价时直接使用，避免每次 Greet 都拉 160 根日 K（自选多时会严重卡顿）。
	// 缺失/错误回填由 BackfillMissingFollowPrices 在启动时异步处理。
	if stored > 0 {
		return stored
	}
	dayClose := receiver.LookupFollowBaselinePrice(follow.StockCode, follow.Time)
	if dayClose <= 0 {
		return 0
	}
	return dayClose
...
func (receiver StockDataApi) isLikelyCorruptedFollowPrice(stored, dayClose float64, followTime time.Time, currentPrice float64) bool {
	if stored <= 0 || dayClose <= 0 {
		return false
	}
	diffFromDay := math.Abs(stored-dayClose) / dayClose
	if diffFromDay <= 0.03 {
		return false
	}
	daysSince := time.Since(followTime).Hours() / 24
	if daysSince < 1 {
		return false
	}
	if currentPrice > 0 {
		nearCurrent := math.Abs(stored-currentPrice)/currentPrice < 0.06
		farFromDay := diffFromDay > 0.08
		if nearCurrent && farFromDay {
			return true
		}
	}
	return diffFromDay > 0.15
}

// BackfillMissingFollowPrices 为缺少关注价的自选回填基准价，并修正明显错误的回填。
func (receiver StockDataApi) BackfillMissingFollowPrices() {
	var follows []FollowedStock
	db.Dao.Model(&FollowedStock{}).Where("is_del = 0").Find(&follows)
	for _, f := range follows {
		if f.Time.IsZero() {
			continue
		}
		dayClose := receiver.LookupFollowBaselinePrice(f.StockCode, f.Time)
		if dayClose <= 0 {
			continue
		}
		if f.FollowPrice <= 0 || receiver.isLikelyCorruptedFollowPrice(f.FollowPrice, dayClose, f.Time, f.Price) {
			db.Dao.Model(&FollowedStock{}).Where("stock_code = ?", f.StockCode).Update("follow_price", dayClose)
		}
	}
}
...
func (receiver StockDataApi) GetCommonKLineData(stockCode string, kLineType string, days int64) *[]KLineData {
	...
	if res["data"] != nil && code == 0 {
		...
		if data != nil {
			...
			case "week":
				if data["qfqweek"] != nil {
					day = data["qfqweek"].([]any)
				} else if data["week"] != nil {
					day = data["week"].([]any)
				}
			case "month":
				if data["qfqmonth"] != nil {
					day = data["qfqmonth"].([]any)
				} else if data["month"] != nil {
					day = data["month"].([]any)
				}
			default:
				if data["qfqday"] != nil {
...
type hotPlateCacheEntry struct {
	codes []string
	at    time.Time
}

var hotPlateConstituentCache sync.Map

type industryListCacheEntry struct {
	list []string
	at   time.Time
}

var industryListCache atomic.Value // industryListCacheEntry

const industryListCacheTTL = 6 * time.Hour
...
func pushStockCodeToSecucode(code string) string {
...
func pushMarketLabel(code string) string {
	switch {
	...
	case strings.HasPrefix(code, "3"):
		return "深交所创业板"
	case strings.HasPrefix(code, "8"), strings.HasPrefix(code, "4"), strings.HasPrefix(code, "9"):
		return "北京证券交易所"
	default:
		return ""
	}
...
// GetHotConceptPlates 东财概念板块排行（按主力净流入），与选股接口同源。
func (receiver StockDataApi) GetHotConceptPlates(limit int) []map[string]any {
	if limit <= 0 || limit > 100 {
		limit = 30
	}
	u := fmt.Sprintf("https://push2.eastmoney.com/api/qt/clist/get?pn=1&pz=%d&po=1&np=1&fltt=2&invt=2&fid=f62&fs=m:90+t:3&fields=f12,f14,f2,f3,f62&wbp2u=|0|0|0|web&_=%d", limit, time.Now().UnixMilli())
	body, err := receiver.push2GET(u)
	if err != nil {
		logger.SugaredLogger.Warnf("GetHotConceptPlates push2GET: %v", err)
		return nil
	}
	var payload struct {
		Data struct {
	...
	out := make([]map[string]any, 0, len(payload.Data.Diff))
	for _, item := range payload.Data.Diff {
		out = append(out, map[string]any{
			"code":      convertor.ToString(item["f12"]),
			"name":      convertor.ToString(item["f14"]),
			"changePct": toFloatAny(item["f3"]),
			"netInflow": toFloatAny(item["f62"]),
		})
	}
	return out
}

// GetHotIndustryPlates 东财行业板块排行（按主力净流入），与 GetHotConceptPlates 同源。
func (receiver StockDataApi) GetHotIndustryPlates(limit int) []map[string]any {
	if limit <= 0 || limit > 100 {
		limit = 30
	}
	u := fmt.Sprintf("https://push2.eastmoney.com/api/qt/clist/get?pn=1&pz=%d&po=1&np=1&fltt=2&invt=2&fid=f62&fs=m:90+t:2&fields=f12,f14,f2,f3,f62&wbp2u=|0|0|0|web&_=%d", limit, time.Now().UnixMilli())
	body, err := receiver.push2GET(u)
	if err != nil {
		logger.SugaredLogger.Warnf("GetHotIndustryPlates push2GET: %v", err)
		return nil
	}
	var payload struct {
		Data struct {
...
func (receiver StockDataApi) push2GET(reqURL string) ([]byte, error) {
	...
	for attempt := 0; attempt < 3; attempt++ {
		...
		req.SetHeader("Host", "push2.eastmoney.com")
		...
		if err != nil {
			lastErr = err
			if isPush2TransientErr(err) {
				logger.SugaredLogger.Infof("push2 重试 attempt=%d url=%s err=%v", attempt+1, reqURL, err)
				continue
			}
			return nil, err
		}
...
func isPush2TransientErr(err error) bool {
	if err == nil {
		return false
	}
...
func emptyAllStocksResp() *models.AllStocksResp {
	out := &models.AllStocksResp{}
	out.Result.Data = []models.StockInfo{}
	return out
}

func (receiver StockDataApi) buildTechnicalIndicatorFilter(technicalIndicators models.TechnicalIndicators) string {
	...
	for i := 0; i < v.NumField(); i++ {
		...
		if value.Kind() == reflect.Bool && value.Bool() {
			...
			if jsonTag != "" {
				indicatorConditions = append(indicatorConditions, fmt.Sprintf("(%s=\"1\")", jsonTag))
			}
		}
		if value.Kind() == reflect.Int && value.Int() != 0 {
			...
			if jsonTag != "" {
				indicatorConditions = append(indicatorConditions, fmt.Sprintf("(%s%s%d)", jsonTag, operator, value.Int()))
			}
		}
	}
	return strings.Join(indicatorConditions, "")
}

func (receiver StockDataApi) buildBaseStockSearch(name, industry string) string {
	search := ""
	if name != "" {
		search += fmt.Sprintf("(SECURITY_NAME_ABBR in (\"%s\"))", strings.TrimSpace(name))
	}
	if industry != "" {
		search += fmt.Sprintf("(INDUSTRY in (\"%s\"))", strings.TrimSpace(industry))
	}
	return search
}

func (receiver StockDataApi) fetchXuanguStocks(page, pageSize int, search, indicators string) *models.AllStocksResp {
...
func buildSecucodeInFilter(codes []string) string {
	...
	return fmt.Sprintf("(SECUCODE in (%s))", strings.Join(parts, ","))
}

func mapPush2RowToStockInfo(row map[string]any, plateName string) models.StockInfo {
	code := convertor.ToString(row["f12"])
	secu := pushStockCodeToSecucode(code)
	return models.StockInfo{
		SECUCODE:         secu,
		SECURITYCODE:     code,
		SECURITYNAMEABBR: convertor.ToString(row["f14"]),
		NEWPRICE:         toFloatAny(row["f2"]),
		CHANGERATE:       toFloatAny(row["f3"]),
		VOLUME:           toFloatAny(row["f5"]),
		DEALAMOUNT:       toFloatAny(row["f6"]),
		TURNOVERRATE:     toFloatAny(row["f8"]),
		HIGHPRICE:        toFloatAny(row["f15"]),
		LOWPRICE:         toFloatAny(row["f16"]),
		PRECLOSEPRICE:    toFloatAny(row["f18"]),
		MARKET:           pushMarketLabel(code),
		CONCEPT:          plateName,
	}
}

func (receiver StockDataApi) plateConstituentSecucodes(plateCode string) []string {
	if v, ok := hotPlateConstituentCache.Load(plateCode); ok {
		entry := v.(hotPlateCacheEntry)
	...
	page := 1
	for page <= 20 {
		u := fmt.Sprintf("https://push2.eastmoney.com/api/qt/clist/get?pn=%d&pz=500&po=1&np=1&fltt=2&invt=2&fid=f3&fs=b:%s&fields=f12&wbp2u=|0|0|0|web&_=%d", page, plateCode, time.Now().UnixMilli())
		body, err := receiver.push2GET(u)
		if err != nil {
			logger.SugaredLogger.Warnf("plateConstituentSecucodes push2GET: %v", err)
			break
		}
		var payload struct {
			Data struct {
		...
		for _, row := range payload.Data.Diff {
			if secu := pushStockCodeToSecucode(convertor.ToString(row["f12"])); secu != "" {
				codes = append(codes, secu)
			}
		}
		if len(payload.Data.Diff) < 500 {
			break
		}
		page++
	}
	hotPlateConstituentCache.Store(plateCode, hotPlateCacheEntry{codes: codes, at: time.Now()})
	return codes
}

func (receiver StockDataApi) getAllStocksByPlateCode(page, pageSize int, plateCode, plateName, name, industry, indicators string) *models.AllStocksResp {
	if indicators != "" {
		return receiver.getAllStocksByPlateCodeWithIndicators(page, pageSize, plateCode, name, industry, indicators)
	}
	u := fmt.Sprintf("https://push2.eastmoney.com/api/qt/clist/get?pn=%d&pz=%d&po=1&np=1&fltt=2&invt=2&fid=f3&fs=b:%s&fields=f12,f14,f2,f3,f5,f6,f8,f15,f16,f17,f18&wbp2u=|0|0|0|web&_=%d", page, pageSize, plateCode, time.Now().UnixMilli())
	body, err := receiver.push2GET(u)
	if err != nil {
		logger.SugaredLogger.Warnf("getAllStocksByPlateCode push2GET: %v", err)
		return emptyAllStocksResp()
	}
	var payload struct {
		Data struct {
	...
	rows := make([]models.StockInfo, 0, len(payload.Data.Diff))
	keyword := strings.TrimSpace(name)
	for _, item := range payload.Data.Diff {
		info := mapPush2RowToStockInfo(item, plateName)
		if keyword != "" && !strings.Contains(info.SECURITYNAMEABBR, keyword) {
			continue
		}
		rows = append(rows, info)
	}
	out := &models.AllStocksResp{Success: true}
	out.Result.Nextpage = page*pageSize < payload.Data.Total
	out.Result.Currentpage = page
	out.Result.Data = rows
	out.Result.Count = payload.Data.Total
	return out
}

func (receiver StockDataApi) getAllStocksByPlateCodeWithIndicators(page, pageSize int, plateCode, name, industry, indicators string) *models.AllStocksResp {
	codes := receiver.plateConstituentSecucodes(plateCode)
	if len(codes) == 0 {
		return emptyAllStocksResp()
	}
	baseSearch := receiver.buildBaseStockSearch(name, industry)
	merged := make([]models.StockInfo, 0, len(codes))
	...
	for i := 0; i < len(codes); i += chunkSize {
		...
		search := baseSearch + buildSecucodeInFilter(codes[i:end])
		res := receiver.fetchXuanguStocks(1, 500, search, indicators)
		for _, row := range res.Result.Data {
			if _, ok := seen[row.SECUCODE]; ok {
				continue
			}
			seen[row.SECUCODE] = struct{}{}
			merged = append(merged, row)
		}
	}
	slice.SortBy(merged, func(a, b models.StockInfo) bool {
		return toFloatAny(a.CHANGERATE) > toFloatAny(b.CHANGERATE)
	})
	total := len(merged)
	if page <= 0 {
		page = 1
	...
	out := &models.AllStocksResp{Success: true}
	out.Result.Nextpage = end < total
	out.Result.Currentpage = page
	out.Result.Data = merged[start:end]
	out.Result.Count = total
	return out
}

func (receiver StockDataApi) GetAllStocks(page int, pageSize int, name string, industry string, concept string, plateCode string, technicalIndicators models.TechnicalIndicators) *models.AllStocksResp {
	indicators := receiver.buildTechnicalIndicatorFilter(technicalIndicators)
	search := receiver.buildBaseStockSearch(name, industry)
	conceptName := strings.TrimSpace(concept)
	plateCode = strings.TrimSpace(plateCode)

	// 热门板块：与 GetHotConceptPlates 同源，用板块代码 b:BKxxxx 拉成份股（避免 CONCEPT 名称与东财选股不一致）
	if plateCode != "" {
		plateLabel := conceptName
		if plateLabel == "" {
			plateLabel = plateCode
		}
		return receiver.getAllStocksByPlateCode(page, pageSize, plateCode, plateLabel, name, industry, indicators)
	}

	if conceptName != "" {
		search += fmt.Sprintf("(CONCEPT in (\"%s\"))", conceptName)
	}
	return receiver.fetchXuanguStocks(page, pageSize, search, indicators)
}
...
// GetAllIndustries 获取所有行业列表（优先本地库，空则回退东财选股实时聚合）
func (receiver StockDataApi) GetAllIndustries() ([]string, error) {
	...
	if cached := receiver.cachedIndustryList(); len(cached) > 0 {
		return cached, nil
	}
	live := receiver.fetchDistinctIndustriesFromXuangu()
	if len(live) > 0 {
		receiver.storeIndustryListCache(live)
		return live, nil
	}
	return industries, err
}

func (receiver StockDataApi) cachedIndustryList() []string {
	v := industryListCache.Load()
	if v == nil {
		return nil
	}
	entry, ok := v.(industryListCacheEntry)
	if !ok || time.Since(entry.at) > industryListCacheTTL || len(entry.list) == 0 {
		return nil
	}
	return entry.list
}

func (receiver StockDataApi) storeIndustryListCache(list []string) {
	if len(list) == 0 {
		return
	}
	copied := append([]string(nil), list...)
	industryListCache.Store(industryListCacheEntry{list: copied, at: time.Now()})
}

func (receiver StockDataApi) fetchDistinctIndustriesFromXuangu() []string {
	set := make(map[string]struct{})
	const pageSize = 2000
	for page := 1; page <= 10; page++ {
		res := receiver.fetchXuanguStocks(page, pageSize, "", "")
		if res == nil || len(res.Result.Data) == 0 {
			break
		}
		for _, row := range res.Result.Data {
			if ind := strings.TrimSpace(row.INDUSTRY); ind != "" {
				set[ind] = struct{}{}
			}
		}
		...
		if len(res.Result.Data) < pageSize {
			break
		}
	}
	if len(set) == 0 {
...
// lookupEastMoneyIndustry 东财行业（与股票信息筛选列表 INDUSTRY 同源）
func (receiver StockDataApi) lookupEastMoneyIndustry(tsCode, symbol string) string {
	emCode := strings.ToUpper(strings.TrimSpace(tsCode))
	if emCode != "" {
		var info models.AllStockInfo
		if err := db.Dao.Model(&models.AllStockInfo{}).
			Where("secucode = ? OR securitycode = ?", emCode, symbol).
			First(&info).Error; err == nil && strings.TrimSpace(info.INDUSTRY) != "" {
			return strings.TrimSpace(info.INDUSTRY)
		}
	}
	secu := emCode
	if secu == "" && symbol != "" {
		secu = strings.ToUpper(ConvertStockCodeToTushareCode(symbol))
	}
	if secu == "" {
		return ""
	}
	res := receiver.fetchXuanguStocks(1, 1, buildSecucodeInFilter([]string{secu}), "")
	if res != nil && len(res.Result.Data) > 0 {
		if ind := strings.TrimSpace(convertor.ToString(res.Result.Data[0].INDUSTRY)); ind != "" {
			return ind
		}
	}
	return ""
...
// industrySectorCacheEntry 行业/板块热路径缓存（避免 Monitor tick 每秒 Lookup/HTTP）
type industrySectorCacheEntry struct {
	industry string
	bkName   string
	at       time.Time
}
...
var (
	industrySectorCache sync.Map // key: lower stockCode -> industrySectorCacheEntry
	// 行业/板块极少日内变化；负缓存同样缓存，防止反复打东财
	industrySectorCacheTTL = 6 * time.Hour
)

// LookupStockIndustrySectorCached 带内存 TTL 的行业/板块解析（行情热路径优先用此）。
func (receiver StockDataApi) LookupStockIndustrySectorCached(stockCode string) (industry, bkName string) {
	key := strings.ToLower(strings.TrimSpace(stockCode))
	if key == "" {
		return "", ""
	}
	if v, ok := industrySectorCache.Load(key); ok {
		if e, ok2 := v.(industrySectorCacheEntry); ok2 && time.Since(e.at) < industrySectorCacheTTL {
			return e.industry, e.bkName
		}
	}
	industry, bkName = receiver.LookupStockIndustrySectorUncached(stockCode)
	industrySectorCache.Store(key, industrySectorCacheEntry{
		industry: industry,
		bkName:   bkName,
		at:       time.Now(),
	})
	return industry, bkName
}

// WarmIndustrySectorCacheAsync 异步预热自选列表的行业/板块，供秒级 Monitor 命中缓存。
func (receiver StockDataApi) WarmIndustrySectorCacheAsync(stockCodes ...string) {
	if len(stockCodes) == 0 {
		return
	}
	go func(codes []string) {
		api := NewStockDataApi()
		for _, c := range codes {
			_, _ = api.LookupStockIndustrySectorCached(c)
		}
	}(append([]string(nil), stockCodes...))
}

// LookupStockIndustrySector 解析行业/板块（带缓存）。行业优先东财 INDUSTRY，板块仍取基础表 BKName。
func (receiver StockDataApi) LookupStockIndustrySector(stockCode string) (industry, bkName string) {
	return receiver.LookupStockIndustrySectorCached(stockCode)
}

// LookupStockIndustrySectorUncached 无缓存解析（仅缓存层内部或强制刷新使用）。
func (receiver StockDataApi) LookupStockIndustrySectorUncached(stockCode string) (industry, bkName string) {
	...
	tsCode := ConvertStockCodeToTushareCode(code)

	var basic StockBasic
	if tsCode != "" {
		if err := db.Dao.Model(&StockBasic{}).Where("ts_code = ?", tsCode).First(&basic).Error; err != nil {
			_ = db.Dao.Model(&StockBasic{}).Where("ts_code = ?", strings.ToUpper(tsCode)).First(&basic).Error
		}
	}
	if basic.ID == 0 && symbol != "" {
		_ = db.Dao.Model(&StockBasic{}).Where("symbol = ?", symbol).First(&basic).Error
	}
	if basic.BKName != "" {
		bkName = basic.BKName
	}

	industry = receiver.lookupEastMoneyIndustry(tsCode, symbol)
	if industry == "" && basic.Industry != "" {
		industry = basic.Industry
	}

	if strings.HasPrefix(code, "hk") && symbol != "" {
		var hk models.StockInfoHK
		if err := db.Dao.Model(&models.StockInfoHK{}).
			Where("code = ? OR code = ?", code, symbol).
			First(&hk).Error; err == nil {
			return industry, hk.BKName
		}
	}

	usCode := code
	if strings.HasPrefix(code, "us") {
		usCode = "gb_" + strings.TrimPrefix(code, "us")
	}
	if strings.HasPrefix(usCode, "gb_") {
		var us models.StockInfoUS
		if err := db.Dao.Model(&models.StockInfoUS{}).Where("code = ?", usCode).First(&us).Error; err == nil {
			return industry, us.BKName
		}
	}
...
func (receiver StockDataApi) AddTradingRecord(record TradingRecord) (uint, error) {
	...
	// 检查频繁交易（草稿不校验）
	if record.Direction == "买入" && record.Status != "draft" {
...
// UpsertSellSignalDraft 止/减信号 → 卖出交易草稿（同日同股覆盖更新）
func (receiver StockDataApi) UpsertSellSignalDraft(record TradingRecord) (uint, error) {
	record.Direction = "卖出"
	record.Status = "draft"
	if record.TradingTime.IsZero() {
		record.TradingTime = time.Now()
	...
	record.TradingTime = record.TradingTime.In(time.Local)
	if record.Volume <= 0 {
		return 0, fmt.Errorf("草稿卖出数量须大于0")
	}
	if record.Price <= 0 {
		return 0, fmt.Errorf("草稿卖出价格须大于0")
	}
	record.Amount = record.Price * float64(record.Volume)
	receiver.fillTradingRecordCloseSnapshot(&record)

	dayStart := time.Date(record.TradingTime.Year(), record.TradingTime.Month(), record.TradingTime.Day(), 0, 0, 0, 0, time.Local)
	dayEnd := dayStart.Add(24 * time.Hour)

	var existing TradingRecord
	err := db.Dao.Model(&TradingRecord{}).
		Where("stock_code = ? AND direction = ? AND status = ? AND trading_time >= ? AND trading_time < ?",
			record.StockCode, "卖出", "draft", dayStart, dayEnd).
		Order("id DESC").
	...
	if err == nil {
		...
		if uerr := db.Dao.Save(&record).Error; uerr != nil {
			logger.SugaredLogger.Errorf("更新卖出草稿失败: %s", uerr.Error())
			return 0, uerr
		}
		return record.ID, nil
	}
	...
	if cerr := db.Dao.Model(&TradingRecord{}).Create(&record).Error; cerr != nil {
		logger.SugaredLogger.Errorf("创建卖出草稿失败: %s", cerr.Error())
		return 0, cerr
	}
	return record.ID, nil
}

// UpsertBuySignalDraft 买点信号 → 买入交易草稿（同日同股覆盖更新）
func (receiver StockDataApi) UpsertBuySignalDraft(record TradingRecord) (uint, error) {
	record.Direction = "买入"
	record.Status = "draft"
	if record.TradingTime.IsZero() {
		record.TradingTime = time.Now()
	...
	record.TradingTime = record.TradingTime.In(time.Local)
	if record.Volume <= 0 {
		return 0, fmt.Errorf("草稿买入数量须大于0")
	}
	if record.Price <= 0 {
		return 0, fmt.Errorf("草稿买入价格须大于0")
	}
	record.Amount = record.Price * float64(record.Volume)
	receiver.fillTradingRecordCloseSnapshot(&record)
	...
	err := db.Dao.Model(&TradingRecord{}).
		Where("stock_code = ? AND direction = ? AND status = ? AND trading_time >= ? AND trading_time < ?",
			record.StockCode, "买入", "draft", dayStart, dayEnd).
		Order("id DESC").
		First(&existing).Error
	...
	if err == nil {
		...
		record.CreatedAt = existing.CreatedAt
		if uerr := db.Dao.Save(&record).Error; uerr != nil {
			logger.SugaredLogger.Errorf("更新买入草稿失败: %s", uerr.Error())
			return 0, uerr
		}
		return record.ID, nil
	}
	...
	if cerr := db.Dao.Model(&TradingRecord{}).Create(&record).Error; cerr != nil {
		logger.SugaredLogger.Errorf("创建买入草稿失败: %s", cerr.Error())
		return 0, cerr
	}
	return record.ID, nil
}

// ConfirmTradingRecordDraft 确认草稿为正式交易记录
func (receiver StockDataApi) ConfirmTradingRecordDraft(id uint) error {
	var record TradingRecord
	if err := db.Dao.Model(&TradingRecord{}).Where("id = ?", id).First(&record).Error; err != nil {
		return err
	}
	if record.Status != "draft" {
		return fmt.Errorf("该记录不是草稿")
	}
	return db.Dao.Model(&TradingRecord{}).Where("id = ?", id).Update("status", "confirmed").Error
}
...
func (receiver StockDataApi) GetTradingRecordList(query TradingRecordListQuery) (*TradingRecordPageData, error) {
	...
	if err := db.Dao.Model(&TradingRecord{}).
		Where("status IS NULL OR status = '' OR status = ?", "confirmed").
		Order("trading_time ASC, id ASC").
		Find(&allGlobal).Error; err != nil {
