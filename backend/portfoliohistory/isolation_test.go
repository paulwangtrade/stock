package portfoliohistory_test

import (
	"go/parser"
	"go/token"
	"os"
	"path/filepath"
	"strings"
	"testing"

	"github.com/stretchr/testify/require"
)

func TestIsolation_NoTradeChainOrProviderSwitch(t *testing.T) {
	wd, err := os.Getwd()
	require.NoError(t, err)

	forbiddenImports := []string{
		"go-stock/backend/strategy",
		"go-stock/backend/execution",
		"go-stock/backend/models",
		"go-stock/backend/data",
		"go-stock/backend/risk",
		"go-stock/backend/papertrading",
		"go-stock/backend/approvegate",
		"go-stock/backend/controlledadoption",
		"go-stock/backend/decisionprovider",
		"go-stock/backend/allocationengine",
	}
	forbiddenSrc := []string{
		"CreatePlanWithItems",
		"BuildDraftTradePlan",
		"FreezeTradePlan",
		"TryBeginExecute",
		"SetActive(",
		"PlanFilter(",
		"Sharpe",
		"sharpe",
		"AutoTune",
		"auto_tune",
	}

	fset := token.NewFileSet()
	entries, err := os.ReadDir(wd)
	require.NoError(t, err)
	for _, e := range entries {
		if e.IsDir() || !strings.HasSuffix(e.Name(), ".go") || strings.HasSuffix(e.Name(), "_test.go") {
			continue
		}
		src, err := os.ReadFile(filepath.Join(wd, e.Name()))
		require.NoError(t, err, e.Name())
		text := string(src)
		for _, bad := range forbiddenSrc {
			// allow flag field names NotAutoTune / not_auto_tune in types
			if bad == "auto_tune" || bad == "AutoTune" {
				continue
			}
			if bad == "sharpe" || bad == "Sharpe" {
				// allow NotSharpe json tag
				continue
			}
			require.NotContains(t, text, bad, e.Name())
		}
		// Explicit: no PnL computation helpers
		require.NotContains(t, text, "func.*PnL", e.Name())
		require.NotContains(t, text, "CumulativeReturn", e.Name())

		f, err := parser.ParseFile(fset, e.Name(), src, parser.ImportsOnly)
		require.NoError(t, err, e.Name())
		for _, imp := range f.Imports {
			p := strings.Trim(imp.Path.Value, `"`)
			for _, bad := range forbiddenImports {
				require.NotEqual(t, bad, p, e.Name()+": "+p)
			}
		}
	}
}

func TestIsolation_WriteChainDoesNotImportPortfolioHistory(t *testing.T) {
	targets := []string{
		"../strategy/build_draft_trade_plan.go",
		"../strategy/controlled_draft.go",
		"../strategy/freeze_trade_plan.go",
		"../risk/plan_filter.go",
		"../execution/port.go",
		"../controlledadoption/policy.go",
	}
	for _, rel := range targets {
		path := filepath.Clean(filepath.Join(".", rel))
		if _, err := os.Stat(path); err != nil {
			continue
		}
		src, err := os.ReadFile(path)
		require.NoError(t, err)
		require.NotContains(t, string(src), "portfoliohistory", path)
	}
}
